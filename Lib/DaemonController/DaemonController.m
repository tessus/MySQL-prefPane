...
//
@interface MSPPDaemonController(/* Hidden Methods */)
...
- (void)start {
	@try {
		...
		[self initDaemonTask];
		daemonTask.launchPath = launchPathStart;
		if (startArguments)
...
- (void)setLaunchPath:(NSString *)theLaunchPath {
	if (launchPath != theLaunchPath) {
		...
		// And set the binary name to the path
